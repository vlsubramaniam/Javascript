...
// Check to see if the data is an Array. If not convert it to an array and then return an array of those arrays. 
// To check the returned value you can display the allArray variable.

let processData = function(...args) {
   return args.map(val => Array.isArray(val) ? val : Array.from(val));
};
